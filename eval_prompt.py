...
def call_llm(prompt, model="haiku"):
    safe_env = {k: os.environ[k] for k in ["PATH", "HOME"] if k in os.environ}
    try:
        result = subprocess.run(
            ["python3", "-m", "llm_judge", "--model", model],
            input=prompt, capture_output=True, text=True, timeout=60, env=safe_env,
        )
...
def main():
    ...
    for i, (sample, label) in enumerate(all_test):
        ...
        scratchpad = sample.get("scratchpad", "")[:1500]
        if not scratchpad:
            parse_failures += 1
            continue
        ...
        if score < 0:
            parse_failures += 1
            continue  # Don't count unparseable as either class
    ...
    print(f"\n{'='*50}", file=sys.stderr)
    print(f"TP={tp} FP={fp} FN={fn} TN={tn} Parse_failures={parse_failures}", file=sys.stderr)
